# 🇰🇷 한국어 机器人 × AI-CLI 브릿지 시스템（bacs-bridge-server）

> 챗봇을 Claude Code / Codex 등 AI CLI의 원격 인터랙션 입구로 변환합니다. 더 이상 서버에 SSH로 접속해 터미널을 열 필요가 없습니다 —— 채팅에서 봇을 @멘션하면 하나 이상의 AI 프로그래밍 프로세스를 직접 구동할 수 있습니다.
...
**bacs-bridge-server**는 챗봇과 AI CLI 도구(Claude Code / Codex)를 양방향으로 브릿징하는 시스템입니다. 상주하는 Bridge Server를 통해 봇 메시지 이벤트를 지정된 CLI 프로세스로 라우팅하고, CLI의 응답을 다시 그룹이나 개인 채팅으로 전송합니다.

적용 시나리오:
- 팀에서 그룹을 통해 협업하며 AI 프로그래밍 작업 구동
- PC가 없는 환경(모바일, 태블릿)에서 서버의 Claude Code 원격 조작
- 동일 서버의 여러 AI 프로세스를 서로 다른 그룹에 바인딩하여 프로젝트 격리
...
| 모듈 | 기능 |
|------|------|
| **다중 머신 바인딩** | 동일 서버에서 여러 CLI 프로세스(cc-a / cc-b / codex-x ...)를 동시에 관리하며, 각 프로세스를 독립적으로 챗봇에 바인딩 |
...
7. 「바인딩」메뉴: 챗봇 바인딩 추가, App ID / App Secret / Verification Token / Encrypt Key + CLI + 서비스 제공자 + 모델 + Effort 선택
8. 그룹에서 봇 @멘션 후 임의 메시지 전송 → 백엔드에서 자동으로 cc/codex 프로세스 시작 및 브릿징
...
저장 후, Bridge가 자동으로 飞书 롱 커넥션 WS 연결을 시도하며, 상태가 `online`이 되면 그룹에서 봇 @멘션을 통해 사용할 수 있습니다.
...
홈페이지 하단 Timeline 영역에 최근 20개 봇 메시지가 표시됩니다（SSE 실시간 푸시）, 새 항목이 상단에서 scale+fade로 슬라이드 인, 클릭으로 전체 내용 펼치기/접기, 플랫폼 태그 색상 구분（飞书 녹색 / Telegram 파란색 예약）.
...
- ⌨️ 스마트폰에서 직접 명령어를 입력해 Bridge로 전송, 채팅 앱을 열 필요 없음

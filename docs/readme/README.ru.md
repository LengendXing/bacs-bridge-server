# Мост Bot × AI-CLI (bacs-bridge-server)

> Превратите чат-ботов в удалённый интерфейс для AI CLI-инструментов, таких как Claude Code / Codex. Больше не нужно подключаться к серверу по SSH и открывать терминал — достаточно упомянуть бота в чате, чтобы управлять одним или несколькими AI-процессами кодирования.
...
**bacs-bridge-server** — это система двусторонней связи между чат-ботами и AI CLI (Claude Code / Codex). Постоянно работающий Bridge Server маршрутизирует события сообщений бота к указанным CLI-процессам и отправляет их ответы обратно в соответствующий чат или группу.

Сценарии использования:
- Команда совместно ведёт задачи AI-кодирования из группового чата
- Удалённое управление Claude Code с телефона / планшета
- Изоляция нескольких AI-процессов на одном сервере, привязанных к разным чатам
...
8. Упомяните бота в группе — процесс `cc` / `codex` стартует автоматически
...
После сохранения Bridge открывает WS-соединение с платформой бота; когда статус становится `online`, можно @-упомянуть бота в чате.
...
Внизу главной страницы — последние 20 сообщений (SSE), новые элементы влетают сверху, кликом разворачиваются/сворачиваются, теги платформ цветовые.
...
- ⌨️ Отправка команд с телефона без открытия чат-приложения
...
**В: Отправил сообщение боту — молчит.**

...
router.get('/api/models', (req, res) => {
    const db = getDb();
    const providerId = req.query.providerId ? parseInt(req.query.providerId, 10) : undefined;
    let result;
...
// ════════════════════════════════════════════════════════════════════
async function refreshProviderModels(providerId, baseUrl, apiKey) {
...
function fetchModelsApi(url, apiKey) {
...
function inferCliKind(modelId, baseUrl) {
    const lower = modelId.toLowerCase();
    const urlLower = baseUrl.toLowerCase();
    if (lower.includes('claude') || urlLower.includes('anthropic'))
        return 'cc';
    if (lower.includes('gpt') || /^o[134]/.test(lower) || urlLower.includes('openai'))
        return 'codex';
    return 'cc';
}

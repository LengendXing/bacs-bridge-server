...
export function buildCliConfig(binding) {
    const db = getDb();
    const cfg = {
        providerKind: 'local',
        envVars: {},
...
export async function startCliProcess(processName, cliKind, cfg, machineId = null) {
...
export async function listAllSessions(machineId) {
    const all = [];
...
export async function listUnboundSessions(machineId) {
    const db = getDb();
    const allBindings = db.select().from(bindings).all();
    const boundNames = new Set(machineId !== undefined

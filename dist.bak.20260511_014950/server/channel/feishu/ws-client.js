...
import { createSession, startProgressTimer, startHardDeadline, startOutputPolling, endSession, hasActiveSession, } from '../../session/state.js';
 ...
 */
function getBindingByAppId(appId) {
    const db = getDb();
    const result = db
        .select()
        .from(bindings)
 ...
 * @returns 绑定记录数组
 */
function getAllBindingsWithFeishu() {
    const db = getDb();
...
function handleIncomingMessage(binding, event) {
    ...
    if (!message || message.message_type !== 'text')
        return;
    if (!feishuAppId || !feishuAppSecret || !cliKind)
        return;
    let msgText = '';
    try {
    ...
    (async () => {
        try {
            ...
            if (!await adapter.sessionExists(processName, executor)) {
                sender
                    .sendText(feishuAppId, feishuAppSecret, targetType, targetId, `进程 [${processName}] 已离线，请先在终端启动 CC 进程`)
                    .catch((e) => logger.log('error', '发送离线提示失败', e.message));
                return;
            }
            if (hasActiveSession(processName)) {
                sender
                    .sendText(feishuAppId, feishuAppSecret, targetType, targetId, `进程 [${processName}] 正在处理上一条消息，请稍后再试`)
            ...
            const result = await adapter.sendInput(`${adapter.sessionPrefix}-${processName}`, msgText, executor);
            if (!result.ok) {
                logger.log('error', '发送消息到进程失败', result.error);
                sender
                    .sendText(feishuAppId, feishuAppSecret, targetType, targetId, `发送到 CC 进程失败: ${result.error}`)
                    .catch((e) => logger.log('error', '发送错误提示失败', e.message));
                endSession(processName);
                return;
            }
            logger.log('info', `消息已路由到进程 ${processName}: ${msgText.slice(0, 60)}`);
            startProgressTimer(session, (s) => {
            ...
            startOutputPolling(session, (s, reply) => {
                if (s.replied)
                    return;
                if (!reply) {
                    sender
                        .sendText(s.ctx.feishuAppId, s.ctx.feishuAppSecret, s.ctx.targetType, s.ctx.targetId, '[CC 已完成处理，但未能提取到回复内容。请检查 tmux 会话或重试]')
 ...
 */
export async function start(binding) {
    const feishuAppId = binding.feishuAppId;
    const feishuAppSecret = binding.feishuAppSecret;
    const { processName } = binding;
    // 飞书凭据为必要字段，缺失则无法建立连接
 ...
 * @returns 已连接返回 true
 */
export function isConnected(appId) {
    const client = clients.get(appId);
...
export async function startAll() {
    ...
    for (const b of allBindings) {
        if (b.feishuAppId && b.feishuAppSecret) {
            start(b).catch((e) => logger.log('error', `启动绑定 WebSocket 失败: ${b.processName}`, e.message));
        }
    }
}

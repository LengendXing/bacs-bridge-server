...
interface ServerConfig {
  ...
  host: string;
  /** Admin dashboard password (overridden by ADMIN_PASSWORD env) */
  adminPassword: string;
}

/** Bridge behaviour tunables */
interface BridgeConfig {
  /** Seconds between progress-push notifications */
  progressInterval: number;
  /** Max seconds to wait for a CLI response */
  timeout: number;
  /** Seconds between CLI output polls */
  pollInterval: number;
...
/** Top-level configuration object returned by {@link load} */
...
export function load(): AppConfig {
  // 0. 从 .env 文件加载环境变量（pm2 / systemd 不自动加载 .env）
  ...
  if (fs.existsSync(yamlPath)) {
    ...
    if (parsed.server) Object.assign(cfg.server, parsed.server);
    if (parsed.bridge) Object.assign(cfg.bridge, parsed.bridge);
    if (parsed.logging) Object.assign(cfg.logging, parsed.logging);
    if (parsed.database) Object.assign(cfg.database, parsed.database);
    if (parsed.jwt) Object.assign(cfg.jwt, parsed.jwt);

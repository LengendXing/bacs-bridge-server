...
import type { Channel, ReplyPayload } from './types.js';
import feishuChannel from './feishu/index.js';

/** 已注册的渠道实例映射: kind → Channel */
const channelRegistry: Map<string, Channel> = new Map([
 ...
 * @returns Channel 实例
 */
export function getChannel(kind: string): Channel | undefined {
 ...
 */
export async function restoreAllChannels(): Promise<void> {
